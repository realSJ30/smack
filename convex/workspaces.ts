...
export const join = mutation({
    args: {
        ...
        if (!workspace) {
            throw new Error("401: Workspace not found!")
        }

        if (workspace.joinCode !== args.joinCode.toLowerCase()) {
            throw new Error("404: Invalid join code!")
        }
        ...
        if (existingMember) {
            throw new Error("401: Already a member of this workspace!")
        }
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // TODO: create a proper method later for join code
        const joinCode = generateCode()
